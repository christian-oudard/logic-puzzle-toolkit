...
class Board(object):    
    ...
    def solve(self, max_depth=2, verify_unique=False):
...
##
#                        print 'contradiction in', conclusion_count
##
                        yield False
                        return
                    yield result
...
##
#                print 'fail in', conclusion_count
##
    ...
    def conclusion_thread(self, depth):
        ...
        for pos in self.unknown_positions:
            for color in (BLACK, WHITE):
                thread_dict[(pos, color)] = self.assumption_thread(pos, color, depth)
        while thread_dict:
            finished_threads = []
            for pos in self.prioritized_positions():
                for color in BLACK, WHITE:
                    try:
                        result = thread_dict[(pos, color)].next()
                    except StopIteration:
                        finished_threads.append((pos, color))
                    except KeyError:
                        continue
